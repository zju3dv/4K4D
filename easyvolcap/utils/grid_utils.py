...
def grid_sample_1d(input: torch.Tensor, grid: torch.Tensor, *args, **kwargs):
    # input: B, C, L_in
    # grid: B, 1, L_out
    ...
    grid = torch.cat([torch.ones_like(grid) / 2, grid], dim=-1)
    z = F.grid_sample(input, grid, *args, **kwargs)  # B, C, L_out, 1
