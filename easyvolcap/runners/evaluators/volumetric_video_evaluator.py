        ...
        if len(self.metrics):
            for key in self.metrics[0].keys():
                ...
                if key == 'time':
                    ...
                    summary[f'{key}{self.skip_time_in_summary:}+_mean'] = np.mean(values).item()
                    summary[f'{key}{self.skip_time_in_summary:}+_std'] = np.std(values).item()
                elif key == 'camera':
                    pass
                elif key == 'frame':
                    pass
                else:
                    summary[f'{key}_mean'] = np.mean(values).item()
                    summary[f'{key}_std'] = np.std(values).item()
