...
if __name__ == "__main__":
    ...
    parser.add_argument('--nerfie_root', type=str, default='data/nerfies/toby-sit')
    parser.add_argument('--easyvv_root', type=str, default='data/nerfies/toby-sit')
    ...
    frame_appear = os.listdir(join(args.nerfie_root, 'camera'))
    ...
    scene_info = json.load(open(join(args.nerfie_root, 'scene.json')))
    ...
    for cam_id, prefix in enumerate(cam_prefix):
        ...
        mocap_camera = {}
        mocap_cam_path = join(args.easyvv_root, 'cameras', f'{cam_id:02d}')  # use 00, 01, 02... 10, otherwise the order will be wrong
        for frame_id, frame_name in enumerate(frame_valids):
            # path generation
            nerfi_cam_path = join(args.nerfie_root, 'camera', f'{prefix}_{frame_name}.json')
    ...
    print('camera processing finish.')
    ...
    for cam_id, prefix in enumerate(cam_prefix):
        ...
        for frame_id, frame_name in enumerate(frame_valids):
            ...
            nerfi_img_path = join(args.nerfie_root, 'rgb', f'{args.ratio}x', f'{prefix}_{frame_name}.png')
            mocap_img_root = join(args.easyvv_root, 'images', f'{cam_id:02d}')
    ...
    nerfi_bkgd_pts_path = join(args.nerfie_root, 'points.npy')
    mocap_bkgd_pts_path = join(args.easyvv_root, 'points.npy')
    ...
    print('background points processing finish.')
