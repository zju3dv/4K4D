...
def main():
    ...
    parser.add_argument('--easyvv_root', type=str, default='data/gaussian/tpose360')
    ...
    run(f'mkdir -p {args.easyvv_root}/images/00')
    run(f'mkdir -p {args.easyvv_root}/masks/00')
    write_camera(cameras, join(args.easyvv_root, 'cameras', '00'))

    for i, heat in enumerate(heatmap):
        heat = heat.sum(axis=-1, keepdims=True).repeat(3, axis=-1)
        save_image(join(args.easyvv_root, 'images', '00', f'{i:06d}.png'), heat)
        save_image(join(args.easyvv_root, 'mask', '00', f'{i:06d}.png'), heat[..., :1] > 0.0001)
